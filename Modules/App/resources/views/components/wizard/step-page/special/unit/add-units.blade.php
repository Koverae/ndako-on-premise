            ...
            <form wire:submit.prevent="submitUnits">
                ...
                <div class="m-0 mb-3 row justify-content-between position-relative w-100">
                    <div class="ke-title mw-75 pe-2 ps-0">
                        <label class="h3" for="name-k">{{ __('To which property do these units belong?') }}</label>
                        <h1 class="flex-row d-flex align-items-center">
                            <select wire:model.live="selectedProperty" id="" class="form-control" id="name-k">
                                <option value="">-- Choose --</option>
                                @foreach ($this->properties as $value => $text)
                                    <option value="{{ $value }}">{{ $text }}</option>
                                @endforeach
                            </select>
                            {{-- <input type="text" wire:model="selectedProperty" class="k-input" id="name-k" placeholder="e.g. Standard Room" > --}}
                            @error('selectedProperty') <span class="text-danger">{{ $message }}</span> @enderror
                        </h1>
                    </div>
                </div>
                ...
                <div class="mb-3 d-flex justify-content-between">
                    <span>&nbsp;</span>
                    <span class="gap-1 btn btn-primary go-next text-end" wire:click="addUnit">{{ __('Add Units') }} <i class="fas fa-plus-circle"></i></span>
                </div>
                ...
                <div class="row {{ $this->propertyUnits ? '' : 'd-none' }}">
                    ...
                    <div class="p-1 mb-1 cursor-pointer col-12 col-lg-4" style="min-height: 122px;">
                        <div class="p-2 border rounded" style="min-height: 122px;">
                            ...
                            <div class="mb-2 mt-2">
                                <span>Property: {{ getProperty($unit['unitProperty'])->name }}</span>
                            </div>
                            <div class="d-flex justify-content-between">
